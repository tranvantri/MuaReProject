...
class ServiceController extends Controller
{
	...
	public function getTinDang($id){
        ...
        ->select('tindang.*',
        'users.id as idChuShop',
        'users.email as emailChuShop',
        'users.phone as phoneChuShop',
        'users.address as addressChuShop',
        'users.name as tenChuShop',
        'users.avatar as avatarChuShop',
		...
		->join('users', 'users.id', '=', 'tindang.idUser')
		->where('users.status',1)
        ->where('tindang.idCate',$service_category->idCate)
        ->where('tindang.status',1)
        ...
        $service_user = DB::table('tindang')
        ->join('users','users.id','=','tindang.idUser')
        ->join('categories','categories.id','=','tindang.idCate')
        ->where('tindang.status',1)
        // ->where('tindang.adminCheck',1)
        ->where('users.status',1)
        ->where('users.id',$getService->idUser)
        ->where('tindang.id','<>',$id)
        ->select('tindang.*','categories.id as idCate','categories.name as nameCate')->get();
        
  		return view('user.chitietsanpham',compact('services','service_category','service_relate','randomProducts','service_user'));
	}
